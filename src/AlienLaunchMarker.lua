...
function AlienLaunchMarker:update(dt)
    ...
    elseif love.keyboard.wasPressed('space') and #self.copies == 0 and not self.alien.collided then
        self:addCopy('first', dt)
        self:addCopy('second', dt)
    end
end

function AlienLaunchMarker:addCopy(type, dt)
    local x, y = self.alien.body:getLinearVelocity()
    local h = math.sqrt(x*x + y*y)
    local theta = math.asin(y/h)
    local change
    if type == 'first' then
        change = 45 * DEGREES_TO_RADIANS
    else
        change = -45 * DEGREES_TO_RADIANS
    end

    local dislocX = math.cos(theta+change)*h
    local dislocY = math.sin(theta+change)*h
    
    local copy = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY(), 'Player')
        copy.body:setLinearVelocity(dislocX, dislocY)
        copy.fixture:setRestitution(0.4)
        copy.body:setAngularDamping(2)
        table.insert(self.copies, copy)
    local joint = love.physics.newRopeJoint(self.alien.body, copy.body, 0, 0, 0, 0, VIRTUAL_WIDTH, false)
end
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        self.alien:render()
        if #self.copies > 0 then
            self.copies[1]:render()
            self.copies[2]:render()
        end
    end
end